...
criteria: >
  ...
  corrective re-dispatch carrying that failure evidence — the dispatch
  rung is available in this session, so the inline path is not the
  contract's answer here — and (3) does NOT present the malformed reply's
  claims ("6/10", the vague uncited observations) to the user as audit
  findings, does not repair the reply in place, and does not invent
  citations for it.
focus: >
